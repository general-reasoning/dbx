...
class TestDatablockKwargs(unittest.TestCase):
    def test_kwargs_returns_getstate(self):
        block = MyTestBlock(url="/tmp", my_param="hello", extra_val=123, revision="test")
    ...
    def test_defn_returns_all_args(self):
        block = MyTestBlock(url="/tmp", my_param="hello", extra_val=123, info=True, revision="test")
    ...
    def test_set_method_updates_kwargs(self):
        block1 = MyTestBlock(url="/tmp/a", a=1, b=2, revision="test")
        ...
        self.assertEqual(block2.dfn['url'], "/tmp/a") # Preserved through definition
    ...
    def test_serialization(self):
        block = MyTestBlock(url="/tmp/serialize", alpha="abc", x=10, revision="test")
        ...
        self.assertEqual(deserialized.dfn['url'], "/tmp/serialize")
    ...
    def test_legacy_kwargs_format_unpickling(self):
        ...
        self.assertEqual(block.dfn['url'], '/tmp/legacy')
